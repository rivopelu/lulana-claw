...
function GenerateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleGenerate = () => {
    ...
    generate.mutate(
      { aiModelId, customPrompt: customPrompt.trim() || undefined },
      { onSuccess: onClose },
    );
  };
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Luna will generate a theme, mood, visual concept, caption, and hashtags for your next
            Instagram post.
          </p>
          <div className="space-y-1.5">
            ...
            {models.length === 0 ? (
              ...
              <Select value={selectedModelId} onValueChange={setSelectedModelId}>
                ...
                <SelectContent>
                  <SelectItem value="__auto__">Auto (use first available)</SelectItem>
                  {models.map((m) => (
          ...
          <div className="space-y-1.5">
            <Label>
              Instruksi Khusus <span className="text-muted-foreground font-normal">(opsional)</span>
            </Label>
            <Textarea
        ...
        <DialogFooter>
          ...
          <Button onClick={handleGenerate} disabled={generate.isPending || models.length === 0}>
        ...
        <button
          ...
          className={[
            ...
            value.includes("instagram")
              ? "border-pink-500 bg-linear-to-r from-pink-500 to-purple-600 text-white"
              : "border-border text-muted-foreground",
...
function ApproveDialog({ draft, onClose }: { draft: ContentDraft | null; onClose: () => void }) {
  ...
  const canSubmit =
    mode === "now" || mode === "approve_only" || (mode === "schedule" && !!schedDate);
...
function PublishDialog({ draft, onClose }: { draft: ContentDraft | null; onClose: () => void }) {
  ...
  return (
    <Dialog open={!!draft} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={publishDraft.isPending}>
            Batal
          </Button>
          <Button
...
function ReviseDialog({ draft, onClose }: { draft: ContentDraft | null; onClose: () => void }) {
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {assets.length === 0 ? (
            <div className="flex h-32 items-center justify-center text-sm text-muted-foreground">
              Gallery kosong — upload media di halaman{" "}
              <strong className="ml-1">Media Gallery</strong>.
            </div>
          ) : (
    ...
    <div className="rounded-lg border bg-card flex flex-col overflow-hidden">
      ...
      <div
        className={`relative h-40 flex items-center justify-center cursor-pointer transition-colors ${
          dragOver ? "bg-primary/10 border-primary border-2" : "bg-muted/30"
        }`}
        onDragOver={(e) => {
          e.preventDefault();
          setDragOver(true);
      ...
      >
        {draft.asset_url ? (
          draft.asset_type === "video" ? (
            <video src={draft.asset_url} className="h-full w-full object-cover" muted />
          ) : (
            <img src={draft.asset_url} alt="Asset" className="h-full w-full object-cover" />
          )
        ) : (
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {isLoading ? (
        ...
        <Tabs
          value={activeTab}
          onValueChange={(v) => setActiveTab(v as ContentDraftStatus | "all")}
        >
          <TabsList>
      ...
      {/* Edit caption dialog */}
      {editTarget && <EditCaptionDialog draft={editTarget} onClose={() => setEditTarget(null)} />}
